...
> **版本说明：** 下列截图来自实际项目中持续使用的完整工程实践版本，并非当前仓库启动后的界面截图。开源仓库提供独立实现的控制平面、Web 控制台、调度与用例发布基础能力，用于复现核心架构、交互设计和端到端闭环；生产版本的全部业务页面、企业认证、数据库适配和真实设备投屏不在开源范围内。

公开版可直接体验概览、任务、模拟设备、用例脑图/列表、发布预览与历史。它保持与实际项目一致的产品思路，但使用公开示例数据和可替换适配器运行。
...
- 独立 Exploration API，不与正式任务队列和租约状态混用
- 追加式事件时间线，可记录结构化工具结果、进度、警告与证据引用
- 探索会话支持完成或整体丢弃，不直接修改正式 Flow
- Agent 负责认证、转发和调度，设备操作与 Flow 编译仍由执行框架负责
